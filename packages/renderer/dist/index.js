...
export function ansiColor(hex, text) {
...
function findSpeaker(doc, speakerId) {
 ...
 * - Weight < 300: dim
 */
function renderWord(word, speakerColor, currentTime) {
...
export class TerminalRenderer {
     ...
     */
    renderEvent(event, speaker, currentTime) {
    ...
    renderSummary(doc) {
        ...
        lines.push(ansiDim(`Duration: ${formatVTTTime(doc.metadata.duration)} | Language: ${doc.metadata.language} | Generator: ${doc.metadata.generator}`));
        lines.push("");
        // Cast
 ...
 */
export function exportWebVTT(doc) {
    const lines = ["WEBVTT", ""];
