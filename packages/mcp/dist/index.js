...
class MockDiarizationBackend {
    async diarize(transcript, _input) {
 ...
 */
class MockIntentExtractorBackend {
    async extract(transcript, _input) {
...
// ============================================================================
function parseCWIDocument(cwi_json) {
    const parsed = JSON.parse(cwi_json);
    // Minimal structural check
    if (!parsed.$schema || !parsed.version || !parsed.metadata || !parsed.cast || !parsed.captions) {
        throw new Error("Invalid CWI document: missing required fields ($schema, version, metadata, cast, captions)");
    }
    return parsed;
}
// ============================================================================
...
// --------------------------------------------------------------------------
server.tool("generate_captions", "Generate CWI (Caption with Intention) captions from a video file. " +
    ...
    try {
        ...
        const result = await pipeline.run({ path: video_path }, {
            language,
            cast_overrides,
        });
...
// --------------------------------------------------------------------------
server.tool("validate_captions", "Validate a CWI document against the 12-rule CWI specification. " +
...
// --------------------------------------------------------------------------
server.tool("preview_captions", "Preview CWI captions as terminal-formatted text. If a time is provided, " +
...
// --------------------------------------------------------------------------
server.tool("export_captions", "Export a CWI document to WebVTT format. WebVTT is the standard " +
