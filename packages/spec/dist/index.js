...
function syn004(doc) {
    ...
    for (const event of doc.captions) {
        for (let i = 0; i < event.words.length; i++) {
            const w = event.words[i];
            if (w.animation?.duration_ms !== undefined &&
    ...
    if (total > 0) {
        ...
        if (percent < CWI_DEFAULTS.INTONATION_MIN_VARIED_PERCENT) {
            findings.push({
                ...
                suggestion: "The intent mapper may not be capturing enough vocal variation. Check pitch extraction.",
            });
        }
    }
    return findings;
...
// ============================================================================
function computePillarScore(findings) {
...
export function validate(doc) {
    ...
    const overallScore = Math.round((attribution.score + synchronization.score + intonation.score) / 3);
    // Count total words
