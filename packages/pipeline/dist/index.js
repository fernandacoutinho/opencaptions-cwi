 ...
 */
export function computeVoiceProfile(frames, speakerId) {
 ...
 */
export function assignSpeakerColors(speakerIds) {
    const map = new Map();
 ...
 */
export const rulesMapper = (word, frame, _speaker, override) => {
    ...
    const baseEmphasis = frame.semantic.emphasis_words.includes(word.text) || frame.vocal.volume_normalized > 0.85;
...
export class Pipeline {
    ...
    async run(input, options) {
        ...
        const cast = speakerIds.map((id, i) => ({
            id,
            name: `Speaker ${i + 1}`,
            ...
            for (const ov of options.cast_overrides) {
                ...
                if (speaker) {
                    if (ov.name !== undefined)
                        speaker.name = ov.name;
                    if (ov.color !== undefined)
                        speaker.color = ov.color;
                }
            }
        }
        ...
        for (const frame of intentFrames) {
            ...
            const frameWords = diarizedTranscript.words.filter((w) => w.speaker_id === frame.speaker_id && w.start >= frame.start && w.end <= frame.end);
            const cwiWords = frameWords.map((dw) => {
