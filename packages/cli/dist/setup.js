 ...
 * Returns exit code.
 */
function runCommandStreaming(cmd, args) {
    return new Promise((resolve) => {
        const child = spawn(cmd, args, {
 ...
 */
async function canImport(module) {
...
export async function cmdDoctor() {
    ...
    if (passedRequired === totalRequired) {
        print(`${GREEN}${BOLD}All ${passedRequired} required components ready.${RESET}${passedOptional < allOptional.length
            ? ` ${DIM}(${passedOptional}/${allOptional.length} optional)${RESET}`
            : ""}`);
    }
    else {
        print(`${YELLOW}${passedRequired} of ${totalRequired} required components ready.${RESET} ` +
